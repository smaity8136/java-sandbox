...
public class DealerOrchestratorImpl implements DealerOrchestrator {
    ...
    public DealerOrchestratorImpl(CarService carService, AccessoryService accessoryService, QuoteService quoteService, LicensingService licensingService, Transformer2<Car, Quote, OTP> otpTransformer) {
    ...
    public Mono<OTP> generateOTP(long carId) {
        return carService.getCar(carId)
                .flatMap(car -> {
                    ...
                    List<Mono<Accessory>> monoList = car.getAccessoryIds().stream().map(accessoryService::getAccessory).collect(Collectors.toList());
                    Mono<Quote> quoteMono = quoteService.getQuote(car.getPurchasePrice());
                    Mono<Boolean> licenseMono = licensingService.driverHasValidLicense(UUID.randomUUID().toString());
                    Mono<Car> zipCarMono = Mono.zip(monoList, args -> {
                        Arrays.stream(args).map(arg -> (Accessory)arg).map(accessory -> car.getAccessories().add(accessory));
                        return car;
                    });
                    return Mono.zip(zipCarMono, quoteMono, licenseMono).map(tuple3 -> otpTransformer.apply(tuple3.getT1(), tuple3.getT2()));
                });
    }
}
